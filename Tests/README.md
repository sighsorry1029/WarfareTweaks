...
- Metadata checks resolve the exact `Humanoid.DrainEquipedItemDurability(ItemData, float)`, six-argument `ItemData.GetTooltip`, and five-argument `SEMan.AddStatusEffect` methods in the loaded Valheim assembly. This confirms target availability, not successful Harmony patch installation or invocation.
